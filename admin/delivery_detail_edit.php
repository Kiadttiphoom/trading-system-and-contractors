                ...
                <div class="container-fluid">
                            ...
                            <div class="card shadow-lg mb-4">
                                ...
                                <div
                                    ...
                                    <h5 class="m-0 font-weight-bold "><font color="#000">ข้อมูลส่งสินค้า</font></h5>
...
if (isset($_POST['update_delivery_detail'])) {

    $userid = $_GET['no_delivery'];
    $id_product = $_POST['id_product'];
    $delivery_number= $_POST['delivery_number'];
    $price_delivery= $_POST['price_delivery'];
    $total_delivery = $_POST['total_delivery'];

    $sql_delivery_detail = $updatedata->update_delivery_detail($id_product, $delivery_number, $price_delivery , $total_delivery ,$userid);
    if ($sql_delivery_detail) {
        echo "<script>alert('แก้ไขข้อมูลสั่งซื้อเรียบร้อย!');</script>";
...
?>       
    <?php 
        ...
        $sql_delivery_detail = $updateuser->fetchonerecord_delivery_detail($userid);
        while($row = mysqli_fetch_array($sql_delivery_detail)) {
    ?>
    <style>
    table, th, td {
  ...
  margin-right: auto;
   ...
   <?php
            ...
            ?>
   <?php
                    $sql = " SELECT delivery.no_delivery, product.name_product, delivery.delivery_number,delivery.price_delivery, delivery.total_delivery
                    FROM delivery 
                    INNER JOIN product  on delivery.id_product  = product .id_product Where no_delivery='$no_delivery'";
            ...
            <tr>
                <div class="mb-3 ">
                        <td style="width:10%"><label for="id_receipt_delivery" class="py-3"><font color="#000"><b>เลขที่ใบเสร็จซื้อ</b></font></label></td>
                        <td style="width:20%"><input class="form-control text-center" name="id_receipt_delivery" value="<?php echo $row['id_receipt_delivery']; ?>" type="text" id="id_receipt_buy"  readonly aria-describedby="text" required style="width:100%"/></td>
            ...
            <tr>        
                    <div class="mb-3">
                    <td style="width:5%"><label for="delivery_number" class="py-3"><font color="#000"><b>จำนวน</b></font></label></td>
                    <td style="width:5%"><input onfocus="sum()"  onblur="sum()" onchange="sum()" onkeyup="sum()" class="form-control text-center" id="delivery_number" name="delivery_number" value="<?php echo $row['delivery_number']; ?>" type="text" aria-describedby="text" required style="width:100%"/></td></td>
                    </div>
            </tr>
            <tr>        
                    <div class="mb-3">
                    <td style="width:5%"><label for="price_delivery" class="py-3"><font color="#000"><b>ราคา</b></font></label></td>
                    <td style="width:5%"><input onfocus="sum()"  onblur="sum()" onchange="sum()" onkeyup="sum()" class="form-control text-center" id="price_delivery" name="price_delivery" value="<?php echo $row['price_delivery']; ?>" type="text" aria-describedby="text" required style="width:100%"/></td></td>
                    </div>
            </tr>
            <tr>        
                    <div class="mb-3">
                    <td style="width:5%"><label for="total_delivery" class="py-3"><font color="#000"><b>ราคารวม</b></font></label></td>
                    <td style="width:5%"><input readonly onfocus="sum()"  onblur="sum()" onchange="sum()" onkeyup="sum()" class="form-control text-center" id="total_delivery" name="total_delivery" value="<?php echo $row['total_delivery']; ?>" type="text" aria-describedby="text" required style="width:100%"/></td></td>
                    </div>
            </tr>
            <script language="javascript">
var obj= document.all;
function sum(){
	obj.total_delivery.value=parseInt(obj.delivery_number.value)*parseInt(obj.price_delivery.value);
}
</script>
		...
		<br>
        <center><button type="submit" name="update_delivery_detail" class="btn btn-success">แก้ไขข้อมูลซื้อ</button>
