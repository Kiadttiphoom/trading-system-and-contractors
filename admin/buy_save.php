...
<?php
  ...
  $status = "1";
   ...
   echo $id_employee; */
      ...
      $sql1 = "INSERT INTO receipt_buy (id_receipt_buy, date_receipt_buy, time_receipt_buy, id_employee, id_company, grand_buy, status) 
      VALUES ('$id_receipt_buy','$date_receipt_buy','$time_receipt_buy','$id_employee','$id_company','$grand_buy','$status')";
...
ini_set('display_errors', 1);
	...
	for ($i = 1; $i<= (int)$_POST["hdnCount"]; $i++){
        ...
        $sql = "INSERT INTO buy (id_receipt_buy, id_product, buy_number, price_buy, total_buy, status) 
					VALUES ('".$_POST["id_receipt_buy"]."','".$_POST["id_product$i"]."','".$_POST["buy_number$i"]."','".$_POST["price_buy$i"]."'
					,'".$_POST["total_buy$i"]."','$status')";
	...
	echo 'alert("เพิ่มข้อมูลการซื้อสำเร็จ")'; 
	echo '</script>';
