<?php
          ...
          $con= mysqli_connect("localhost","root","","thailand") or die("Error: " . mysqli_error($con));
  ...
  if (isset($_POST['function']) && $_POST['function'] == 'provinces') {
  	...
  	foreach ($query as $value) {
  		echo '<option value="'.$value['id'].'">'.$value['name_amphures'].'</option>';
...
if (isset($_POST['function']) && $_POST['function'] == 'amphures') {
    ...
    foreach ($query as $value2) {
      echo '<option value="'.$value2['id'].'">'.$value2['name_districts'].'</option>';
