            ...
            <div id="content">
                ...
                <div class="container-fluid" style="width:80%">
                            ...
                            <div
                                    ...
                                    <center><h5 class="m-0 font-weight-bold "><font color="#000">แก้ไขข้อมูลสินค้า</h5></center>                           
                                </div>
                                <!-- Card Body -->
...
if (isset($_POST['update_product'])) {
    ...
    $sql_product = $updatedata->update_product($name_product, $id_type_product, $price_sell,$img_product ,$userid);
    if ($sql_product) {
        echo "<script>alert('แก้ไขข้อมูลสินค้าเรียบร้อย!');</script>";
        echo "<script>window.location.href='product.php'</script>";
    } else {
        echo "<script>alert('บางอย่างผิดพลาด กรุณาลองอีกครั้ง!');</script>";
        echo "<script>window.location.href='product_edit.php'</script>";
    }
}
...
?>       
    <?php 
        ...
        $sql_product = $updateuser->fetchonerecord_product($userid);
  ...
  margin-right: auto;
   ...
   <center> <table style="width:50%" class="center text-center"> 
            ...
            <tr>
                <div class="mb-3 ">
                        <td style="width:10%"><label for="id_product" class="py-3"><font color="#000"><b>รหัสสินค้า</b></font></label></td>
                        <td style="width:20%"><input class="form-control text-center" name="id_product" value="<?php echo $row['id_product']; ?>" type="text" id="id_product"  readonly aria-describedby="text" required style="width:100%"/></td>
                        
                </div>
            </tr>
            <tr>        
                    <div class="mb-3">
                    <td style="width:5%"><label for="name_product" class="py-3"><font color="#000"><b>ชื่อสินค้า</b></font></label></td>
                    <td style="width:5%"><input class="form-control text-center" name="name_product" value="<?php echo $row['name_product']; ?>" type="text" id="name_product"  aria-describedby="text" required style="width:100%"/></td></td>
                    </div>
            </tr>
            <tr>
            
                    <td style="width:5%"><label for="id_type_product" class="py-3"><font color="#000"><b>ประเภทสินค้า</b></font></label></td>
                    <td style="width:10%"><select class="id_type_product form-control text-center" style="width:100%" id="id_type_product" name="id_type_product">
                                    <?php
                                            $sql = " SELECT * FROM type_product ORDER BY id_type_product ASC";
                                            $result = $objCon->query($sql);
                                            
                                        while($data = $result->fetch_assoc()):
                                    ?>        
                                    <option value="<?php echo $data['id_type_product']?>"><?php echo $data['name_type_product']?></option>
            ...
            <tr>        
                    <div class="mb-3">
                    <td style="width:5%"><label for="price_sell" class="py-3"><font color="#000"><b>ราคาขาย</label></td>
                    <td style="width:5%"><input class="form-control text-center" id="price_sell" name="price_sell" value="<?php echo $row['price_sell']; ?>" type="text" aria-describedby="text" required style="width:100%"/></td></td>
                    </div>
            </tr>

            <tr >
                        <td style="width:10%"><label  class="form-label py-3" for="form1Example1"><font color="#000"><b>หน่วยนับ</b></font></label></td>
                            <td><select class="form-control text-center" style="width:100%" id="id_unit" name="id_unit">
                                    <?php
                                            $sql2 = " SELECT * FROM unit ORDER BY id_unit ASC";
                                            $result2 = $objCon->query($sql2);
                                            
                                        while($data2 = $result2->fetch_assoc()):
                                    ?>        
                                    <option value="<?php echo $data2['id_unit']?>"><?php echo $data2['name_unit']?></option>
                        ...
                        <tr >
                            <td><label class="form-label py-3" for="form1Example1"><font color="#000"><b>รูปภาพ</b></font></label></td>
                            <td><input name="img_product" type="file" id="img_product" class="" placeholder="รูปภาพ" required/></td>
                        </tr>
		...
		<br>
        <center><button type="submit" name="update_product" class="btn btn-success">แก้ไขข้อมูล</button>
        <a href="product.php" type="submit" name="update" class="btn btn-danger">ยกเลิก</a></center>
        <br>
