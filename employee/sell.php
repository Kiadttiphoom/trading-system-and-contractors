            ...
            <div id="content">
                ...
                <div class="container-fluid">
                            ...
                            <div class="card shadow-lg mb-4">
                                ...
                                <a href="/project/employee/sell_add.php" class="btn btn-success pull-right" type="button" style="width:%"><i class="fa fa-plus" aria-hidden="true"></i> เพิ่มข้อมูลขายสินค้า</a>
                <!-- Fixed header table-->
  ...
  border: 0.5px solid;
    ...
    <table id="dataTable" class="table table-striped table-hover" >
            ...
            <thead style="background-color:#f26b5f;">
                <tr class=" " >
                    <th><center><font size=2 color=#FFFFFF>เลขที่ใบเสร็จขาย</font></center></th>
                    ...
                    <th><center><font size=2 color=#FFFFFF>วันที่ขาย</font></center></th>
                    <th><center><font size=2 color=#FFFFFF>เวลาที่ขาย</font></center></th>
                    <th><center><font size=2 color=#FFFFFF>ราคารวมทั้งหมด</font></center></th>
                    <th><center><font size=2 color=#FFFFFF>สถานะ</font></center></th>
                    <!-- <th><center><font size=2 color=#FFFFFF>เปลี่ยนสถานะ</font></center></th> -->
                    <th><center><font size=2 color=#FFFFFF><i class="fas fa-align-justify"></font></center></th>
                </thead>
            <tbody>
            <?php
                    $sql = " SELECT receipt_sell.id_receipt_sell , employee.name_employee, customer.name_customer, customer.tel_customer, receipt_sell.date_receipt_sell, receipt_sell.time_receipt_sell, receipt_sell.grand_sell, receipt_sell.status
                    FROM receipt_sell 
                    INNER JOIN employee on receipt_sell.id_employee = employee.username 
                    INNER JOIN customer on receipt_sell.id_customer = customer.id_customer ORDER BY receipt_sell.id_receipt_sell";
                    $result = $objCon->query($sql);
                    while($row = $result->fetch_assoc()):

                        $sql1 = " SELECT  SUM(total_sell) AS grand_sell
                        FROM receipt_sell 
                        INNER JOIN sell ON receipt_sell.id_receipt_sell=sell.id_receipt_sell
                        WHERE sell.id_receipt_sell = '$row[id_receipt_sell]';";
            ...
            ?>
                      ...
                      <tr>
                          ...
                          <td><center><font size=2 color="#000"><b><?php echo $row['id_receipt_sell']; ?></b></font></center></td>
                          ...
                          <td><center><font size=2 color="#000"><b><?php echo $row['date_receipt_sell']; ?></b></font></center></td>
                          <td><center><font size=2 color="#000"><b><?php echo $row['time_receipt_sell']; ?></b></font></center></td>
                          <td><center><font size=2 color="#000"><b><?php echo $row1['grand_sell']; ?></font></center></td>
                          <td class="text-center" ><font size=2><b><!-- <a href="sell_change.php?Action=Change&id_receipt_sell=<?php echo $row['id_receipt_sell']; ?>" onclick="return confirm('ยืนยันการเปลี่ยนสถานะ')"> -->
                                <?php if($row ['status']=='1'){ 
                                    echo "<font color=\"green\">ขายสำเร็จ</font>";
                                    }
                                    elseif($row ['status']=='0') { 
                                        echo "<font color=\"orange\">ยกเลิกการขาย</font>";
                                    } 
                                    else { 
                          ...
                          <!-- <td>
                            <form action="sell_change.php?id_receipt_sell=<?php echo $row['id_receipt_sell']; ?>" method="post">
                          <select name='status' id='status'>
                            <option value = '1'>ขายสำเร็จ</option>
                            <option value = '2'>รอส่ง</option>
                          ...
                          <button type="button" class="btn btn-primary btn-sm"
                                                                    href="sell-buy/buyadd.php"><a
                                                                        href="print/report_bill_sell.php?id_receipt_sell=<?php echo $row['id_receipt_sell']; ?>"
                                                                        target="_Blank" class="text-white"><i
                                                                            class='fas fa-print fa-lg'></i>
                                                                        </a></button>
                          <a href="sell_detail.php?id_receipt_sell=<?php echo $row['id_receipt_sell']; ?>"><button type="button" class="btn btn-success btn-sm"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#"><i
                                                                         class='fa fa-info-circle far-lg'></i></button></a>
                         <!--  <td><center><a href='edit.php?id_product=<?php echo $row['id_product']; ?>&name_product=<?php echo $row['name_product']; ?>&id_type_product=<?php echo $row['id_type_product']; ?>&price_sell=<?php echo $row['price_sell']; ?>&price_sell=<?php echo $row['price_sell']; ?>'><button type='button' class='btn btn-success'>แก้ไข</button></a></center></td> -->
                        
                          <a href="sell_receipt_delete.php?id_receipt_sell=<?php echo $row['id_receipt_sell']; ?>"><button type="button" class="btn btn-danger btn-sm"
                                                                        data-bs-toggle="modal"
                                                                        data-bs-target="#delete_data"><i
                                                                            class='fas fa-trash-alt far-lg'></i></button></a></center></td>
